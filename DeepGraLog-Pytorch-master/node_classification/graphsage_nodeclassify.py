...
class DataCenter_test(object):
    ...
    def load_Dataset(self, dataset='test'):
        ...
        test_index, val_index = self._split_data(feat_list.shape[0])
    ...
    def _split_data(self, number_of_nodes, test_split=6, val_split=3):
        """
            Split the test set and validation set
        """
        ...
        val_index = rand_indices[test_size:]
...
class GraphSage(nn.Module):
    ...
    def aggregate(self, nodes, pre_hidden_embs, pre_neighs, num_sample=1):
        """
...
def evaluate(dataCenter, ds, graphSage, classification, device, name, cur_epoch):
    ...
    embs = graphSage(val_nodes, feature_data, adj_lists)
    logists = classification(embs)
    _, predicts_val = torch.max(logists, 1)
    labels_val = labels[val_nodes]
    assert len(labels_val) == len(predicts_val)
    comps = zip(labels_val, predicts_val.data)
    labels_val = list(labels_val)
    predicts_val = predicts_val.data.numpy().tolist()
    ...
    return labels_val,predicts_val,labels_test,predicts_test
